...
def predict1():
  if request.method=='POST':
    img1=request.files['ima1'].read()
    
    file_bytes = np.asarray(bytearray(img1), dtype=np.uint8)
    img1 = cv2.imdecode(file_bytes, 1)
    img1 = cv2.resize(img1, (48,48))
    img1.shape = (1,48,48,3)
    u1=model1.predict(img1)
    pre1=processesing(u1)
    print(u1)
    perc1=percentage(u1,pre1)
    
    if pre1==0:
      print(0)
      response1="You are Safe,No Covid"
      return render_template("prediction1.html",predict1=response1,percent1=str(perc1)+" %")

    if pre1==1:
      print(1)
      response1="You are not safe,Yes you have Covid"
      return render_template("prediction1.html",predict1=response1,percent1=str(perc1)+" %")
